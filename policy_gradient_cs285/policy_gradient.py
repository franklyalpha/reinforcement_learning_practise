...
def train_policy_network(training_configs, device, env, policy_net, optimizer):
    ...
    for training_epoch in range(epochs):
        ...
        gradient = torch.sum(-log_probs_record * reward_to_go.to(device)) / batch_size # never forget the negative sign!!!
        optimizer.zero_grad()
        gradient.backward()
        optimizer.step()
        # calculate average reward
        print("epoch {} with reward {}".format(training_epoch, torch.mean(reward_record)))
